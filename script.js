...
function renderBox(){const n=state.current.length;$("boxNumber").textContent=state.boxes.length+1;$("boxCount").innerHTML=`${n}<small>/12</small>`;$("progressMessage").textContent=progressMessage();$("progressMessage").classList.toggle("complete",n===12);$("subtotal").textContent=money(n*PRICE);$("segments").innerHTML=Array.from({length:12},(_,i)=>`<i class="${i<n?"on":""}"></i>`).join("");$("slots").innerHTML=Array.from({length:12},(_,i)=>state.current[i]?`<div class="slot full" data-index="${i}"><img src="${ASSET_BASE}/${state.current[i]}.svg" alt="${state.current[i]}"><button aria-label="Remover ${state.current[i]}">×</button><small>${state.current[i]}</small></div>`:`<div class="slot"><i>${i+1}</i><small>livre</small></div>`).join("");document.querySelectorAll(".slot.full").forEach(slot=>slot.querySelector("button").onclick=()=>{state.current.splice(Number(slot.dataset.index),1);save();render()});const confirm=$("confirmBox");confirm.disabled=n!==12;confirm.textContent=n===12?`Concluir Caixa ${state.boxes.length+1} →`:`Faltam ${12-n} para completar`;$("clearBox").hidden=n===0;$("mobileBoxCount").textContent=`${n}/12`;$("mobileBoxTotal").textContent=money(n*PRICE)}
function renderSaved(){const count=state.boxes.length;$("headerBoxes").textContent=`Caixa ${count+1}`;$("headerUnits").textContent=`${state.current.length}/12`;$("savedBoxes").hidden=count===0;$("savedBoxesList").innerHTML=state.boxes.map((box,i)=>`<div class="saved-row"><div class="saved-thumbs">${box.slice(0,4).map(sku=>`<img src="${ASSET_BASE}/${sku}.svg" alt="">`).join("")}<i>+8</i></div><span>✓ Caixa ${i+1} guardada</span><b>12 peças · R$ 479,88</b></div>`).join("");$("grandTotal").textContent=`${count} caixa${count===1?"":"s"} · ${count*12} peças · ${money(count*12*PRICE)}`;$("sendOrder").disabled=count===0;$("checkout").hidden=!(state.checkoutVisible&&count>0)}
